...
private:
    ...
    std::map<std::string, std::string> mCities;
    ...
    void AddCountry (const Attributes &);
...
void SaxHandler::characters (const XMLCh * const chars, const XMLSize_t length)
{
    ...
    if (inCity && inCountry && inCityName && !seenCityName)
        AddCity (data);
                             ...
                             const XMLCh * const qname)
{
    ...
    if (inCityName && std::strcmp (tag, "name") == 0)
    {
        inCityName = false;
        seenCityName = true;
    }
...
void SaxHandler::AddCountry (const Attributes &attrs)
{
    for (int i=0; i<attrs.getLength(); ++i)
        {
            auto attribute = XMLString::transcode (attrs.getQName (i));
            

            if (std::strcmp (attribute, "name") == 0)
            {
                auto value = XMLString::transcode (attrs.getValue (i));
                currentCountry = value;
                mCountries.push_back (value);
...
void SaxHandler::CreateCitiesXML ()
{
    ...
    for (const auto &city : mCities)
    {
        ...
        cityXML << "\t\t<city-name>" << mCities[country] << "</city-name>\n";
